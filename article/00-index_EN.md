...
First, use CoreCoder, a seventeen-hundred-line open source project, to lay out clearly how a production-grade coding agent like Claude Code is built on the inside. Second, walk you through forking CoreCoder and turning it into a coding agent of your own.
...
3. [Plug in any model, and get the bill right while you're at it](03-llm-and-cost_EN.md). How `llm.py` (335 lines, the largest single file in the project) uses one OpenAI-compatible interface to catch DeepSeek, Qwen, Kimi, and local Ollama, how it does exponential-backoff retry, and how it tallies tokens and dollar cost right inside the streaming output.
