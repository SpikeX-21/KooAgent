...
第一，借 CoreCoder 这个一千七百行的开源项目，把 Claude Code 这类生产级编码 agent 的内部结构讲清楚。第二，带你把 CoreCoder fork 下来，改出一个属于你自己的 coding agent。
...
CoreCoder 做的事，是把这套骨架压到一千七百行纯 Python。准确说是 1712 行物理代码，去掉空行和注释一千三百多行。每个文件都短到能一口气读完，跑起来又确实是个能改代码、能跑命令、能自己开子任务、能压缩上下文、能算钱的 agent。它不是玩具。它是把生产级 agent 的每个设计决策，拣出最核心的那版，用最少的代码诚实地写一遍。
...
3. [接入任意大模型，顺便把钱算清楚](03-llm-and-cost.md)。`llm.py`（335 行，全系列最大的文件）怎么用一套 OpenAI 兼容接口接住 DeepSeek、Qwen、Kimi、本地 Ollama，怎么做指数退避重试，怎么在流式输出里顺手把 token 和美元成本统计出来。
