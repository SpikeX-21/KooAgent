...
CoreCoder 里就有一个现成的例子值得拿来当反面教材，bash 的工作目录跟踪。上一篇之前讲过，bash 需要在多次命令之间记住 `cd` 去了哪，所以它把当前目录存了起来。问题在于，它存的位置是一个模块级的全局变量：
...
干净的解法是把这种「每个执行流各自的状态」隔离开，别让它们共享。Python 里现成的工具是 `threading.local()`，它给每个线程一份独立的副本，线程之间互不可见：
...
我把这一段单独拎出来讲，是因为它是个特别好的教学点：**当你给 agent 加并行，你就同时给所有带可变状态的工具加了一道并发正确性的要求**。这条要求平时藏得很深，单看 bash 那个文件你看不出问题，单看并行那个函数你也看不出问题，只有把「bash 用全局存 cwd」和「这些工具会被并发调用」两件事摆在一起，隐患才浮出来。这正是第四篇讲孤儿 tool 消息时那种跨越多个文件的不变式，最阴险的 bug 几乎都长这样。如果你打算 fork CoreCoder 往里加会写状态的工具，这是你必须先想清楚的一课：你的工具，扛得住被两个线程同时调用吗？
...
- 并行不是免费的：它给每个带可变状态的工具都加了一道并发正确性要求。bash 用模块全局存 cwd 在并行下会 race，正解是用 `threading.local` 把状态隔离到线程。
